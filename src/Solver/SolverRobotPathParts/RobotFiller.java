...
public class RobotFiller implements Comparable<RobotFiller>
{
	...
	//public final Point robotDestination;
	...
	public RobotFiller(MovePoint start, MovePoint end, int requiredRobots, Directions changedDirection, boolean robotOnEnd, RobotPathPart containerOfThisFiller)
	{
		...
		//this.robotDestination = (robotOnEnd) ?  new Point(end.x, end.y) : new Point(end.x + changedDirection.translationX, end.y + changedDirection.translationY);
	}
	...
	public boolean addPath(RouteBuildingInformation routeInfo)
	{
		...
		for (int q = 0; q < fillerPoints.length; q++)
		{
			...
			int robotsThatCanReachFillerCount = 0;
			for (int i = 0; i < containerOfThisFiller.robotsToBuildPathWith.length; i++)
			{
				...
				if (isBotMoveable[robot])
				{
					...
					if (goalPoint != null)
					{
						robotIndexForRobotsThatCanReachFiller[robotsThatCanReachFillerCount] = robot;
						robotIndexLength[robotsThatCanReachFillerCount] = goalPoint.depth;
						robotIndexDepthPointer[robotsThatCanReachFillerCount] = goalPoint;
						robotsThatCanReachFillerCount++;
					}
				}
			}
			...
			else
			{
				...
				int index = 1;
				while (index < robotsThatCanReachFillerCount)
				{
		...
		if (robotOnEnd)
		{
			expectedStopPoint = new Point(start.x + changedDirection.translationX, start.y + changedDirection.translationY);
		}
		else
		{
			expectedStopPoint = new Point(start.x, start.y);
		}
		
		//haven't implemented blocking yet so for now it will just fail when a robot is in the way
		if (!moverStopPoint.equals(expectedStopPoint))
		{
			return false;
		}
		...
		//need to fix so any robot can be the target and not just robot 0
		//routeInfo.botPositions[0] = new Point(robotDestination.x, robotDestination.y);
		routeInfo.botPositions[0] = expectedStopPoint;
	...
	private BotPathDirection[] getRoutePart(RouteBuildingInformation routeInfo, final Point[] botPositions, Point goal, DepthPointer[] robotIndexDepthPointer, int bestRobot, int index)
	{
		...
		{
			DepthPointer interceptPoint = robotIndexDepthPointer[index];
	...
	private DepthPointer getRobotInterceptingPoint(RouteBuildingInformation routeInfo, final Point[] botPositions, Directions robotBuildDirection, int previousRobotFiller, Point goal, Map<Integer, DepthPointer> robotDepthMap)
	{
		...
		else
		{
			if (previousRobotFiller != -1)
			{
				goalPoint = RobotPath.getInterceptingDepthPointer(routeInfo.board, robotDepthMap, botPositions[previousRobotFiller].x, botPositions[previousRobotFiller].y, robotBuildDirection.getOppositeDirection());
			}
		}
		return goalPoint;
	}

	private Map<Integer, DepthPointer> getRobotDepthMap(RouteBuildingInformation routeInfo, final Point[] botPositions, Point goal, final int robot, final Point botPosition)
	{
		...
		if (routeInfo.botsDepthMap[robot] != null)
		{
			...
			if (lowestPoint == null || lowestPoint.depth != 0)
			{
				robotDepthMap = null;
